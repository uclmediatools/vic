...
V4l2Device::V4l2Device(const char *dev, const char *name, char *attr) 
: InputDevice(name)
{
	dev_ = dev;
	attributes_ = attr;
...
V4l2Grabber::V4l2Grabber(const char *cformat, const char *dev)
{
	...
	if (-1 == ioctl(fd_,VIDIOC_QUERYCAP,&cap)) {
		...
		if ( !(cap.flags & V4L2_FLAG_READ) && 
			...
			fprintf(stderr,"v4l2: fatal: device does not support read() call 
					or streaming capture.\n");
			return;
		}
			...
			for (i = 0, err = 0; err == 0; ++i) {
				...
				if (!err) {
					...
					for ( s=0 ; s<= strlen(inp.name) ; s++ )
						if (inp.name[s]==' ') input[s]='-'; 
						else input[s]=inp.name[s];
	...
	if (fd_ > 0 ) {
		...
		if (have_mmap) {
			...
			for (i = 0; i < req.count; ++i) {
				...
				vimage[i].data = (typeof(vimage[0].data)) 
					mmap(0,  vimage[i].vidbuf.length, PROT_READ, MAP_SHARED, 
							fd_, vimage[i].vidbuf.offset);

				if ((int)vimage[i].data == -1) {
					DEBUG(fprintf(stderr,"v4l2: mmap() returned error %d\n", 
								errno));
					return;
				} else DEBUG(fprintf(stderr,"v4l2: mmap()'ed buffer at 
							0x%x (%d bytes)\n", 
			...
			vimage[0].data = (typeof(vimage[0].data)) 
				malloc(fmt.fmt.pix.sizeimage);
			...
			if (vimage[0].data == NULL) {
				...
				DEBUG(fprintf(stderr,"v4l2: malloc()'ed buffer (%d bytes)\n", 
							fmt.fmt.pix.sizeimage));
		}
				...
				if (!err) {
					...
					if (fmtd.pixelformat == V4L2_PIX_FMT_YUYV) {
						...
						if ( ( fmt.fmt.pix.width != (unsigned int)width_ ) ||
							...
							DEBUG(fprintf(stderr,"v4l2: failed to set format! 
										requested %dx%d, got %dx%d\n", 
										width_, height_, fmt.fmt.pix.width, 
										fmt.fmt.pix.height));
							...
							DEBUG(fprintf(stderr,"v4l2: setting format: 
										width=%d height=%d\n", 
		...
		if ( !(qctrl.flags & V4L2_CTRL_FLAG_DISABLED) ) {
			...
			if ( reset )
				...
				ctrl.value = qctrl.minimum + 
					(qctrl.maximum - qctrl.minimum) * val/256;
