...
int btree_add    (btree_t *tree, u_int32_t key, void *data);
...
int btree_get_min_key (btree_t *tree, u_int32_t *key);
int btree_get_max_key (btree_t *tree, u_int32_t *key);

/* btree_get_next_key attempts to get the key above cur_key.  Returns        */
/* TRUE and fills key with key if possible, FALSE otherwise.                 */

int btree_get_next_key (btree_t *tree, u_int32_t cur_key, u_int32_t *next_key);
